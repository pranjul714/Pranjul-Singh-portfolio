...
const ParticleBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const randomizeParticle = (p: Partial<Particle>): Particle => {
    ...
    function createParticle(x = 0, y = 0, z = 0): Particle {
      return {
        x, y, z,
        vx: 0, vy: 0,
        pastZ: 0,
    ...
    const loop = () => {
      ...
      for (let i = 0; i < PARTICLE_NUM; i++) {
        ...
        const f = FL / p.z;
        p.screenX = centerX + (p.x - centerX) * f;
        p.screenY = centerY + (p.y - centerY) * f;
