...
export default function About() {
  const statsData = [
    { label: "Experience", value: "Fresher", icon: <Award size={20} /> },
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="relative flex justify-center order-2 lg:order-1">
          <div className="relative w-[320px] h-[380px] lg:w-[420px] lg:h-[500px] flex items-center justify-center">
            ...
            >
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3 h-3 bg-zinc-100 rounded-full shadow-[0_0_15px_rgba(255,255,255,0.4)]" />
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 h-2 bg-zinc-400 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.2)]" />
            ...
            >
              ...
              <motion.img
                ...
                className="relative z-10 max-h-[85%] object-contain filter grayscale opacity-90 transition-all hover:grayscale-0"
              />
            </motion.div>
          </div>
        </div>
        ...
        <div className="space-y-8 order-1 lg:order-2">
          ...
          <motion.p
            className="text-lg text-zinc-400 leading-relaxed max-w-xl font-medium"
          >
            I am a passionate Full-Stack Developer focused on building
            scalable web applications with clean architecture and elegant
            user experiences. Specializing in modern React and Node.js ecosystems.
          </motion.p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 pt-4">
            {statsData.map((stat, index) => (
              ...
              >
                <div className="p-3 bg-white/5 text-white rounded-2xl mb-3">
                  {stat.icon}
                </div>
                <h3 className="text-2xl font-black text-white">{stat.value}</h3>
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1 font-bold">{stat.label}</p>
              </motion.div>
            ))}
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="bg-white hover:bg-zinc-200 text-black px-10 py-4 rounded-2xl font-bold shadow-xl transition-all"
          >
            Learn More
          </motion.button>
        </div>
